...
def answer_user_question_combined(system_text, db, user_question, question_history):
    ...
    if filtered_docs:
        ...
        completion = client.chat.completions.create(
            ...
            messages=messages,
            temperature=0.3,
            max_tokens=4000  # Ограничиваем количество токенов
        )
...
def summarize_questions(dialog):
    ...
    messages = [
        ...
        {"role": "user", "content": f"Саммаризируй только краткую тему диалога менеджера поддержки и пациента, не более 1000 токенов: " + " ".join(dialog)}
    ]
